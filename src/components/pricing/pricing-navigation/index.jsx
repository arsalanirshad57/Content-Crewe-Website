...
const PricingNavigation = ({ data, onClick, className, categoryOpen, setCategoryOpen, ...others }) => {
    return (
        <div
            className={`flex justify-start items-center w-fit bg-light-black px-8 py-3 rounded-full lg:mb-5  ${className}`}
            {...others}
        >
            {
                data?.map((item, index) => (
                    <div key={item?.id} className=' flex justify-center items-center gap-2 cursor-pointer' >
                        {index !== 3 ? (
                            <>
